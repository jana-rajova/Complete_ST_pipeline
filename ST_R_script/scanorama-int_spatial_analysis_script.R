...
he.image = readJPEG('CN53_C1_HE.jpg')
...
load(file = 'CN53_C1_exp_values')
load(file = 'CN53_C1_sce')
load(file = 'CN53_C1_log2_norm_exp_values') 
load(file = 'CN53_C1_log2_norm_exp_values_matrix')
...
bw.image = readJPEG('CN53_C1_mask.jpg')
...
scanorama_data = data.frame(read.csv("dataset-dimred_CN53_C1_expdata.csv"))
colnames(scanorama_data) <- -1:99
...
cluster.colors.matrix = c('red', 'blue', 'magenta', 'orange',"hotpink", 'sienna' ,'firebrick', 'darkorchid', 'darkslateblue','goldenrod','coral', 'darkkhaki','darkred'. 'darkseagreen', 'darkmagenta' , 'maroon', 'Cadetblue', 'yellow', 'cyan', 'burlywood', 'black', 'purple', 'dark grey')
...
out.file = paste(sample, '_clusters_scanorama.pdf', sep = '')
pdf(out.file, onefile = TRUE, useDingbats = FALSE, height = 35 * 0.3, width = 33 * 0.3)
...
out.file = paste(sample, '_clusters_interpolated_scanorama.pdf', sep = '')
pdf(out.file, onefile = TRUE, useDingbats = FALSE, height = y * 0.075, width = x * 0.075)
