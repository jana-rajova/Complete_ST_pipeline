...
if __name__ == "__main__":
	...
	if not os.path.isdir("../AE"):
		os.mkdir("../AE")
	adata = desc.train(adata, dims=[adata.shape[1], 32, 16], tol=0.005, n_neighbors=3, batch_size=256, louvain_resolution=[0.9], save_dir="../AE", do_tsne=True, learning_rate=300, do_umap=True, num_Cores_tsne=4, save_encoder_weights=False)
	prob_09=adata.uns["prob_matrix0.9"]
	adata.obs["max.prob0.9"]=np.max(prob_09,axis=1)
	adata.write("anndata_DESC_clustered.h5ad")
	#tSNE plot 
	sc.pl.scatter(adata,basis="tsne0.9",color=['desc_0.9',"max.prob0.9",'TH', 'SLC6A3'], show=False, save="DESC_tSNE.png")
	#Umap plot 
	sc.pl.scatter(adata,basis="umap0.9",color=['desc_0.9',"max.prob0.9",'TH', 'SLC6A3'], show=False, save="DESC_UMAP.png")
