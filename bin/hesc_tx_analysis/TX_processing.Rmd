...
```{r, orthologs}
transfile <- "../data/human_gene_translations.txt"
# get ensembl ids for the human genes
gene.info <- read.table("../data/gene_annotation.tsv", sep="\t", quote='', header=T)
...
exprs.filtered <- exprs[, M.filtered$ClusterName]

# extract all their marker genes
markersA <- unique(unlist(sapply(as.vector(M.filtered$MarkerGenes), strsplit, " ")))
#length(markersA)
# and our marker genes
markers2 <- unique(markers[markers$p_val_adj<0.001,"gene"])
#length(markers2)
...
cC <- cor(exprsC,exprsA, method = "spearman")
include.annot <- c("TaxonomyRank1","TaxonomyRank2","TaxonomyRank3","TaxonomyRank4", "ClusterName")
colorsC <- M.filtered[,include.annot]
  
# keep order for atlas cells by their Taxonomy ranks
o <- order(M.filtered$TaxonomyRank1, M.filtered$TaxonomyRank2, M.filtered$TaxonomyRank3, M.filtered$ClusterName)
pheatmap(cC[,o], annotation_col = colorsC[,1:4], cluster_cols = F, show_colnames = F, fontsize = 6) 
```
```{r, fig.width=10, fig.height=6}
cC <- cor(exprsC,exprsA, method = "spearman")
...
# keep order for atlas cells by their Taxonomy ranks
o <- order(M.filtered$TaxonomyRank1, M.filtered$TaxonomyRank2, M.filtered$TaxonomyRank3, M.filtered$Description)
pheatmap(cC[,o], annotation_col = colorsC[,1:2], cluster_cols = F, show_colnames = F, fontsize = 6) 
```
