...
deconvolution.folder <- c('/media/Dropbox/MNM projects/Spatial transcriptomics project/Data analysis/Complete_ST_pipeline/results/cell2loc_res/L5_CTX_M_STR_description_selection_2000_astro-merge/TX/cell2location_map/')

deconv.file <- paste0(deconvolution.folder, 'celltype_feature.tsv')
...
for (sample_ in samples){
  ...
  for (cell.type in cell.types) {
    ...
    if (!is.na((max(cell.type.exp[,1])))) {
      ...
      dir.create(paste0(deconvolution.folder, 'visuals_interpolation/', sample_, '/', 'PDF/relative/'), showWarnings = FALSE, recursive = TRUE)
      out.file = paste(deconvolution.folder, 'visuals_interpolation/', sample_, '/', 'PDF/relative/', sample_, '_', cell.type.file,'_relative_interpolated.pdf', sep = '')
      pdf(out.file, onefile = TRUE, useDingbats = FALSE, height = y * 0.3, width = x * 0.3)
      
      plot(section.value[,1], -section.value[,2], cex = 0.1, pch = 19, col = alpha(dat.col, 1), xlab='', ylab='', xaxt='n', yaxt='n', axes=FALSE)
      
      dev.off()
      
      dir.create(paste0(deconvolution.folder, 'visuals_interpolation/', sample_, '/', 'PNG_ctrl/relative/'), showWarnings = FALSE, recursive = TRUE)
      out.file = paste(deconvolution.folder, 'visuals_interpolation/', sample_, '/', 'PNG_ctrl/relative/', sample_, '_', cell.type.file,'_relative_interpolated.png', sep = '')
      png(out.file)
      
      plot(section.value[,1], -section.value[,2], cex = 0.2, pch = 11, col = alpha(dat.col, 1), xlab='', ylab='', xaxt='n', yaxt='n', axes=FALSE) + title(paste(strwrap(cell.type, width = 55), collapse = "\n"))
      
      dev.off()
      
      # do the same but replace relative min max with an universal one
      
      min = min(sample.deconv.df[cell.types])
      max = max(sample.deconv.df[cell.types])
      ...
      dir.create(paste0(deconvolution.folder, 'visuals_interpolation/', sample_, '/', 'PDF/absolute/'), showWarnings = FALSE, recursive = TRUE)
      out.file = paste(deconvolution.folder, 'visuals_interpolation/', sample_, '/', 'PDF/absolute/', sample_, '_', cell.type.file,'_absolute_interpolated.pdf', sep = '')
      pdf(out.file, onefile = TRUE, useDingbats = FALSE, height = y * 0.3, width = x * 0.3)
      ...
      dir.create(paste0(deconvolution.folder, 'visuals_interpolation/', sample_, '/', 'PNG_ctrl/absolute/'), showWarnings = FALSE, recursive = TRUE)
      out.file = paste(deconvolution.folder, 'visuals_interpolation/', sample_, '/', 'PNG_ctrl/absolute/', sample_, '_', cell.type.file,'_absolute_interpolated.png', sep = '')
      png(out.file)
